		...
		for(DRPItem item : toRegister){
			if(item.getSubNames() != null){
				...
				for(int i = 0; i < item.getSubNames().length; i++){
					locations[i] = new ResourceLocation(item.getRegistryName().getNamespace(), item.getItemFolder() + "/" + item.getSubNames()[i]);
				}
				...
				ModelBakery.registerItemVariants(item, new ResourceLocation(item.getRegistryName().getNamespace(), path));
				ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(item.getRegistryName().getNamespace() + ":" + path, "inventory"));
			}
		}
		toRegister.clear();
	}
