...
public class CraftingManager {
	...
	public static ArrayList<String> getCategoryList(Block craftingStation) {
		...
		if(CategoryList.containsKey(craftingStation)){ return CategoryList.get(craftingStation); }
		return null;
	}
	...
	public static CraftingRecipe getRecipe(Block Station, String Category, int RecipeID) {
		...
		if(CategoryRecipeList == null) return null;
	...
	public static boolean craftItem(int stationID, String craftCategory, int recipeID, EntityPlayer player) {
		...
		if(Recipe != null){
			...
			for(ItemStack Input : MainIngredients){
				if(!hasItemStack(Input, player, Recipe.shouldIgnoreMeta())){ return false; }
			}
			...
			for(ItemStack Input : MainIngredients){
				for(int i = 0; i < Input.stackSize; ++i){
					...
					if(getInventorySlotContainItem(Input.getItem(), player) >= 0) removedItem = playerInv.getStackInSlot(getInventorySlotContainItem(Input.getItem(), player));
	...
	private static int getInventorySlotContainItem(Item itemIn, EntityPlayer player) {
		...
		for(int i = 0; i < player.inventory.mainInventory.length; ++i){
			if(player.inventory.mainInventory[i] != null && player.inventory.mainInventory[i].getItem() == itemIn){ return i; }
		}
