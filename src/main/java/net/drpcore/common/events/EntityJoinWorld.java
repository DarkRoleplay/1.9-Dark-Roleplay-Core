...
public class EntityJoinWorld {
	 ...
	 * { if(!event.getWorld().isRemote){ if(event.getEntity() instanceof
	 * EntityPlayer){
	 * if(event.getEntity().hasCapability(DarkRoleplayCore.DRPCORE_INV, null)){
	 * AdvancedPlayerInventory inventory =
	 * event.getEntity().getCapability(DarkRoleplayCore.DRPCORE_INV,
	 * null).getInventory(); for(int i = 0; i < inventory.getSizeInventory();
	 * i++){ PacketHandler.sendTo(new PacketSyncAdvancedInventory(i,
