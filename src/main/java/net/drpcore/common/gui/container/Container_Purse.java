...
public class Container_Purse extends Container {
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int index) {
		...
		if(slot != null && slot.getHasStack()){
			...
			if(index < INV_START){
				 ...
				 * between action bar and inventory Be sure to choose only ONE
	...
	/*
	 * @Override public ItemStack slotClick(int slot, int button, int flag,
	 * EntityPlayer player) { if (slot >= 0 && getSlot(slot) != null) { return
	 * super.retrySlotClick(slot, button, flag, player); } else { return null; }
	 * }
	 */
}
