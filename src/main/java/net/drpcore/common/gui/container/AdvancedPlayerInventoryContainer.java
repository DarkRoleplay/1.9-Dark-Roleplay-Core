...
public class AdvancedPlayerInventoryContainer extends Container {
	...
	private final AdvancedPlayerInventory inventory2;
		...
		if(slot != null && slot.getHasStack()) {
			...
			if(klicked > HOTBAR_START - 1 && klicked <= INV_END) {
				...
				if(itemstack1.getItem() instanceof NecklaceBase) {
					if( ! this.mergeItemStack(itemstack1, SPECIAL_NECKLACE, SPECIAL_NECKLACE + 1, false))
						return null;
				} else if(itemstack1.getItem() instanceof RingBase) {
					if( ! this.mergeItemStack(itemstack1, SPECIAL_RING_LEFT, SPECIAL_RING_RIGHT + 1, false))
						return null;
				} else if(itemstack1.getItem() instanceof PurseBase) {
					if( ! this.mergeItemStack(itemstack1, SPECIAL_PURSE, SPECIAL_PURSE + 1, false))
						return null;
				} else if(itemstack1.getItem() instanceof AmmunitionBase) {
					if( ! this.mergeItemStack(itemstack1, SPECIAL_AMMUNITIONCONTAINER, SPECIAL_AMMUNITIONCONTAINER + 1, false))
						return null;
				}
				// if item is armor
				/**
				 * else if (itemstack1.getItem() instanceof ItemArmor){ int type
				 * = ((ItemArmor) itemstack1.getItem()).armorType; if
				 * (!this.mergeItemStack(itemstack1, ARMOR_START + type,
				 * ARMOR_START +1 + type, false))return null; }
				 **/
	...
	public void onContainerClosed(EntityPlayer player) {
		...
		// {
		// player.dropPlayerItemWithRandomChoice(inventoryplayer.getItemStack(),
		// false);
