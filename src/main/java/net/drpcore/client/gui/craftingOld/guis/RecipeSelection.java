...
public class RecipeSelection extends DRPGuiScreen {
	...
	private SelectRecipeButton[] chooseRecipe = new SelectRecipeButton[18];
    ...
    public void initGui(){
		...
		for(int j = 0; j < 3; j ++){
			for(int i = 0; i < 6; i++){
				chooseRecipe[(j * 6) + i] = new SelectRecipeButton(buttonID++, this.guiLeft + 12 + (i * 26), this.guiTop + 64 + (j * 26));
			}
		}
	...
	private boolean isMouseOverButton(SelectRecipeButton btn, int mouseX, int mouseY)
    {
