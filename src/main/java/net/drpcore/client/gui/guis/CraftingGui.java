...
public class CraftingGui extends GuiContainer {
	...
	public CraftingGui(Container container, EntityPlayer player, Block craftingStation) {
		...
		if(this.CategoryList != null) this.categoryName = this.CategoryList.get(this.currentCategoryID);
	...
	public void initGui() {
		...
		if(recipe.getMainIngredients() != null) this.maxIngredients = recipe.getMainIngredients().length;
		if(recipe.getAdditionalIngredients() != null) this.maxAddIngredients = recipe.getAdditionalIngredients().length;
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		switch (button.id) {
			...
			case 2:
				...
				if(this.selectedRecipe - 1 < 0) this.prevPage.enabled = false;
				updateIngredients();
				break;
			// Next Recipe
			case 3:
				...
				if(this.selectedRecipe + 2 > this.recipeCount) this.nextPage.enabled = false;
				updateIngredients();
				break;
			// Prev Ingredients
			case 4:
	...
	private void drawCategory() {
		...
		if(categoryName != null) this.mc.fontRendererObj.drawString(categoryName, x + 88 - ((categoryName.length() / 2) * 5), y + 8, 0xffFFFFFF);
	}
	...
	private void drawList(float partialTicks, int x, int y) {
		...
		if(this.selectedRecipe - 1 >= 0){
			...
			if(recipe1 != null) drawItemStack(recipe1.getOutput(null), x0, y0, 2);
		}
		...
		if(this.selectedRecipe + 1 < this.recipeCount){
			...
			if(recipe3 != null) drawItemStack(recipe3.getOutput(null), x2, y2, 2);
		}
	...
	private void drawIngredients(CraftingRecipe recipe, int x, int y) {
		...
		for(int i = 0; i < 5; i++){
			if(this.currentIngredients + i < mIngr.length) drawItemStack(mIngr[this.currentIngredients + i], x + 66 + (18 * i), y + 33, hasItemStack(mIngr[this.currentIngredients + i], recipe.shouldIgnoreMeta()));
		}
		if(aIngr != null){
			for(int i = 0; i < 5; i++){
				if(this.currentAddIngredients + i < aIngr.length) drawItemStack(aIngr[this.currentAddIngredients + i], x + 66 + (18 * i), y + 60, hasItemStack(aIngr[this.currentAddIngredients + i], recipe.shouldIgnoreMeta()));
			}
		}
	...
	private void drawItemStack(ItemStack stack, int x, int y, int hasNumber) {
		...
		if(stack != null){
			...
			else if(hasNumber == 0) this.itemRender.renderItemOverlayIntoGUI(font, stack, x, y, TextFormatting.RED + stackSizeString);
			this.zLevel = 0.0F;
